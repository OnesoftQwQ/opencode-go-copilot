...
| **驾驶舱上下文进度**            | 通过 `LanguageModelDataPart` (MIME `"usage"`) 向 Copilot Chat 驾驶舱上报 token 用量，实时显示上下文窗口使用进度                                                                                                                        |
| **工具预激活 (Tool Preflight)** | 实验性功能。在 API 请求前主动调用 VS Code 的 `activate_*` 虚拟工具，稳定跨轮次的 tools 列表，提高 DeepSeek KVCache 命中率                                                                                                              |
...
provideLanguageModelChatResponse(model, messages, options, progress, token)
  │
  ├── 0. 工具预激活 (Tool Preflight)
  │       └── processToolFlow()
  │           ├── 过滤 preflight 控制流消息和通知
  │           ├── 若 stabilizeToolList 启用:
  │           │   ├── inspectActivatePreflight() 检测未激活的 activate_* 工具
  │           │   ├── 有未激活工具 → progress.report(LanguageModelToolCallPart)
  │           │   └── 短路返回 (等待 Copilot 重新发起带完整工具的请求)
  │           ├── 所有工具已激活 → 检测工具漂移，注入通知
  │           └── 返回 { preflightHandled, messages, initialResponseNotice }
  │
  ...
  │     │       └── 用量统计 (usage chunk)
  │     │           └── reportCopilotContextUsage(progress, usage) → 上报驾驶舱
  │     │
  │     └── Anthropic 模式:
  ...
  │             └── message_delta → 累计 output_tokens + reportCopilotContextUsage()
  │
  ├── 11. 图片代理拦截处理:
...
### 2.7 工具预激活 (Tool Preflight) 流程

```
processToolFlow({ stabilizeToolList, messages, tools, progress })
  │
  ├── filterPreflightControlFlow(messages)  ← 移除前轮 preflight 产生的 tool_call/tool_result
  ├── filterProviderNotices(messages)       ← 移除前轮注入的工具漂移通知
  │
  ├── stabilizeToolList === false?
  │   └── YES → 返回 { preflightHandled: false, messages: filtered }
  │
  ├── inspectActivatePreflight(messages, tools)
  │   ├── collectActivateToolNames(tools)     ← 收集所有 "activate_*" 前缀工具
  │   ├── findLatestHumanUserMessageIndex()    ← 定位最新用户消息
  │   ├── 扫描之后的消息中的 preflight tool_call/tool_result
  │   ├── calledActivatorNames: 已调用的激活工具
  │   └── remainingActivatorNames: 尚未激活的工具
  │
  ├── remainingActivatorNames.length > 0?
  │   ├── YES & rounds < MAX_PREFLIGHT_ROUNDS:
  │   │   ├── for each toolName in remainingActivatorNames:
  │   │   │   └── progress.report(LanguageModelToolCallPart)
  │   │   │        callId = "opencodego-preflight-activate:N:toolName"
  │   │   └── 返回 { preflightHandled: true, messages }
  │   │       (不调用 API，等待 Copilot 处理激活并重新发起请求)
  │   └── YES & rounds >= 3 → 放弃，继续请求
  │
  └── 返回 { preflightHandled: false, messages, initialResponseNotice? }
```

关键常量：`MAX_PREFLIGHT_ROUNDS_PER_USER_REQUEST = 3`、`ACTIVATE_TOOL_PREFIX = "activate_"`。
...
reportCopilotContextUsage(progress, usage)
  ...
  ├── 序列化为 JSON → Uint8Array
  └── progress.report(LanguageModelDataPart(data, "usage"))
      └── VS Code Copilot Chat 识别 "usage" MIME 类型
          └── 自动更新驾驶舱上下文窗口显示
```

调用时机：

- **OpenAI 模式**: 在 `processStreamingResponse()` 解析到 usage chunk 时立即调用
- **Anthropic 模式**: 在 `processAnthropicChunk()` 收到 `message_delta` 事件时累计 input/output tokens 后调用
...
├── tools/
│   ├── consts.ts                         # 工具预激活常量定义
│   ├── preflight.ts                      # activate_* 预激活检测逻辑
│   ├── notices.ts                        # 工具漂移通知
│   └── flow.ts                           # 工具流处理编排
├── cockpit.ts                            # 驾驶舱上下文进度上报
└── zen/
...
| `tools/consts.ts`                     | ~15  | 工具预激活常量 (`ACTIVATE_TOOL_PREFIX`, `PREFLIGHT_ACTIVATE_CALL_ID_PREFIX`, `MAX_PREFLIGHT_ROUNDS_PER_USER_REQUEST`, `DEEPSEEK_TOOLS_LIMIT`) |
| `tools/preflight.ts`                  | ~145 | activate\_\* 预激活检测逻辑（收集未激活工具、过滤控制流、创建 preflight ID）                                                                  |
| `tools/notices.ts`                    | ~45  | 工具漂移通知生成和过滤                                                                                                                        |
| `tools/flow.ts`                       | ~90  | 工具流处理编排（`processToolFlow`）                                                                                                           |
| `cockpit.ts`                          | ~40  | 驾驶舱上下文进度上报（`reportCopilotContextUsage`），通过 `LanguageModelDataPart` (MIME `"usage"`) 向 Copilot Chat 发送 token 用量            |
...
### 4.24 `src/tools/consts.ts`

#### `const DEEPSEEK_TOOLS_LIMIT = 128`

DeepSeek API 单次请求最大 tools 数量限制。

#### `const ACTIVATE_TOOL_PREFIX = "activate_"`

Copilot 虚拟工具的统一前缀。

#### `const PREFLIGHT_ACTIVATE_CALL_ID_PREFIX = "opencodego-preflight-activate:"`

预激活工具调用的 callId 前缀。

#### `const MAX_PREFLIGHT_ROUNDS_PER_USER_REQUEST = 3`

每次用户请求最多进行的预激活轮数。

---

### 4.25 `src/tools/preflight.ts`

#### `interface ActivatePreflightInspection`

`{ rounds, calledActivatorNames, remainingActivatorNames }` — 预激活检查结果。

#### `inspectActivatePreflight(messages, tools): ActivatePreflightInspection`

检查当前消息历史和工具列表，找出哪些 `activate_*` 工具尚未被调用。

#### `filterPreflightControlFlow(messages): readonly LanguageModelChatRequestMessage[]`

从消息列表中移除预激活控制流产生的 tool_call/tool_result 消息。

#### `createPreflightToolCallId(round, toolName): string`

创建预激活工具调用的唯一 ID（格式：`opencodego-preflight-activate:N:toolName`）。

---

### 4.26 `src/tools/notices.ts`

#### `createToolDriftNotice(): string`

创建工具漂移警告通知文本。

#### `filterProviderNotices(messages): readonly LanguageModelChatRequestMessage[]`

从消息列表中移除之前注入的工具漂移通知。

---

### 4.27 `src/tools/flow.ts`

#### `interface ToolFlowOptions`

`{ stabilizeToolList, messages, tools, progress }` — 工具流处理选项。

#### `interface ToolFlowResult`

`{ preflightHandled, messages, initialResponseNotice? }` — 工具流处理结果。

#### `processToolFlow(options: ToolFlowOptions): ToolFlowResult`

工具流处理核心编排函数。当 `stabilizeToolList` 启用时，检查未激活的 `activate_*` 工具并通过 `progress.report()` 发出预激活调用；短路返回等待 Copilot 重新发起请求。稳定后检测工具漂移并注入通知。

---

### 4.28 `src/cockpit.ts`

#### `reportCopilotContextUsage(progress, usage): void`

向 Copilot Chat 驾驶舱上报 token 用量。构建包含 `prompt_tokens`、`completion_tokens`、`total_tokens`、`prompt_tokens_details.cached_tokens` 的 JSON payload，通过 `LanguageModelDataPart` 以 MIME `"usage"` 发送。VS Code Copilot Chat 识别该 MIME 类型后自动更新上下文窗口显示。
