...
function isUsageTooltipEnabled(): boolean {
	return vscode.workspace
		...
		.get<boolean>("showUsageInTooltip", true);
}
...
function getUsageRefreshIntervalMs(): number {
	const minutes = vscode.workspace
		.getConfiguration("opencodego")
		.get<number>("usageRefreshInterval", 5);
...
export function initStatusBar(
	...
	context.subscriptions.push(
		vscode.workspace.onDidChangeConfiguration((e) => {
			if (
				e.affectsConfiguration("opencodego.showUsageInTooltip") ||
				e.affectsConfiguration("opencodego.usageRefreshInterval")
			) {
				startUsagePolling();
...
export async function updateContextStatusBar(
	...
	try {
		...
		const hasAssistantMessages = messages.some(
			(m) => (m.role as unknown as number) === ASSISTANT,
		);
...
function appendGoUsageTooltipLines(lines: string[]): void {
	...
	const present = windows.filter(
		(entry): entry is [string, GoUsageWindow] => entry[1] !== undefined,
	);
	if (present.length === 0) {
		return;
	}
 ...
 * and data is cached).
 */
export function updateCumulativeTooltip(
	statusBarItem: vscode.StatusBarItem,
): void {
	const arrowUp = "\u2191";
	const arrowDown = "\u2193";
