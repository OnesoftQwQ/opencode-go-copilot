...
function getRequestedReasoningEffort(
	...
	const modelOptions = (
		options as unknown as { modelOptions?: Record<string, unknown> }
	).modelOptions;
	const modelOptionsThinking = modelOptions?.thinking as
		| { type?: unknown }
		| undefined;
	...
	const modelOptionsEffort =
		modelOptions?.reasoning_effort ?? modelOptions?.reasoningEffort;
	return typeof modelOptionsEffort === "string"
		? modelOptionsEffort
		: undefined;
}
...
export class OpenCodeGoChatModelProvider implements LanguageModelChatProvider {
	...
	private _createFetchWithTimeout(requestTimeoutMs: number): typeof fetch {
		try {
			...
			const undici = require(
				path.join(vscode.env.appRoot, "node_modules", "undici"),
			);
	...
	async provideLanguageModelChatResponse(
		...
		try {
			...
			let um: OpenCodeGoModelItem | undefined = {
				...getCatalogModelConfig(model.id),
			};
			...
			if (um) {
				if (um.supportsTemperature !== false) {
					const tempPreset = config.get<string>(
						"opencodego.modelPreset",
						"custom",
					);
					if (tempPreset !== "custom") {
						const presets = config.get<ModelPreset[]>(
						...
						const userTemperature = config.get<number | null>(
						...
						const userTopP = config.get<number | null>(
							"opencodego.top_p",
							null,
						);
						if (userTopP !== null) {
			...
			const enableThirdPartyIndicator = config.get<boolean>(
			...
			const estimatedInputTokens = await updateContextStatusBar(
				messages,
				options.tools,
				this.statusBarItem,
				modelConfig,
			...
			logger.debug("request.headers", {
				headers: logger.sanitizeHeaders(
			...
			if (apiMode === "anthropic") {
				...
				const anthropicMessages = await anthropicApi.convertMessages(
					messages,
					modelConfig,
				);
				...
				const response = await executeWithRetry(async () => {
					...
					if (!res.ok) {
						...
						console.error(
							"[Anthropic Provider] Anthropic API error response",
							errorText,
						);
				...
				await anthropicApi.processStreamingResponse(
					response.body,
					trackingProgress,
					token,
				);
				...
				const openaiMessages = await openaiApi.convertMessages(
					messages,
					modelConfig,
				);
				...
				await openaiApi.processStreamingResponse(
					response.body,
					trackingProgress,
					token,
				);
			...
			if (!usageReportedDuringStream) {
				...
				const estimatedOutputTokens = outputText
					? await textTokenLength(outputText)
					: 0;
			...
			if (
				...
				resolveProviderForModelId(model.id) === "opencode"
			) {
				...
				throw new Error(
					l10nFormat(
						"{0} is no longer available as a free model. Please use a different model.",
						zenModelName,
					),
				);
			}
			...
			if (errMessage.includes("IMAGE_SENSITIVE:")) {
				...
				throw new Error(
					l10n(
						"The image you sent was flagged as sensitive by the content moderation system. Please try a different image.",
					),
				);
			}
	...
	private async _handleInterceptedToolCall(params: {
		...
		const storedMessages = (api as any)._originalApiMessages as
			| any[]
			| undefined;
		...
		if (!hasLocalImages) {
			logger.debug("vision.no-stored-images", {
		...
		for (let round = 1; round <= maxRounds; round++) {
			...
			try {
				if (intercepted.name === ASK_WITH_MULTI_IMAGE_TOOL_NAME) {
					...
					if (images.length < 2) {
						...
						description = await callVisionModelMulti(
							images,
							visionModelId,
							visionPrompt,
							params.token,
					...
					// Single image
					const storedImage = api.getStoredImage(
						intercepted.args.imageIndex ?? 0,
					);
					if (!storedImage) {
						logger.warn("vision.image-not-found", {
							imageIndex: intercepted.args.imageIndex,
						});
			...
			params.trackingProgress.report(
				createVisionToolHistoryPart(
					historyEntry,
				) as unknown as LanguageModelResponsePart,
			);

			if (params.token.isCancellationRequested) {
				logger.info("vision.skipped-round", {
					round,
			...
			const roundTimeoutMs = vscode.workspace
				.getConfiguration()
				.get<number>("opencodego.requestTimeout", 600000);
			...
			try {
				if (params.apiMode === "anthropic") {
					...
					if (
						params.um?.temperature !== undefined &&
						params.um.temperature !== null
					) {
						if (params.um.supportsTemperature !== false) {
							body.temperature = params.um.temperature;
						}
					}
					const systemContent = (params.api as any)._systemContent as
						| string
						| undefined;
					if (systemContent) {
					...
					if (hasLocalImages) {
						const singleDef = ASK_IMAGE_TOOL_DEF as unknown as {
							function: {
								name: string;
								description: string;
								parameters: object;
						...
						if (((api as any)._localImages as any[])?.length >= 2) {
							const multiDef = ASK_WITH_MULTI_IMAGE_TOOL_DEF as unknown as {
								function: {
									name: string;
									description: string;
									parameters: object;
					...
					if (
						params.um?.enable_thinking !== false &&
						params.um?.reasoning_effort !== undefined &&
						params.um.reasoning_effort !== "adaptive"
					) {
