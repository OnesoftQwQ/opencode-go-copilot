...
import {
	...
	generateCommitMsg,
} from "./gitCommit/commitMessageGenerator";
...
const WALKTHROUGH_ID =
	"OnesoftQwQ.opencode-go-copilot-provider#opencodeGoGettingStarted";
...
export function activate(context: vscode.ExtensionContext) {
	...
	const tokenCountStatusBarItem: vscode.StatusBarItem = initStatusBar(
		context,
		context.secrets,
	);
	const provider = new OpenCodeGoChatModelProvider(
		context.secrets,
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("opencodego.setApiKey", async () => {
			...
			const apiKey = await vscode.window.showInputBox({
				...
				prompt: existing
					? l10n("Update your OpenCode Go API key")
					: l10n("Enter your OpenCode Go API key"),
				ignoreFocusOut: true,
				password: true,
				value: existing ?? "",
			});
			...
			if (!apiKey.trim()) {
				...
				vscode.window.showInformationMessage(
					l10n("OpenCode Go API key cleared."),
				);
				return;
			}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("opencodego.updateModelList", async () => {
			try {
				...
				resetAutoDiscoveryState();
				await prepareLanguageModelChatInformation(
					{ silent: true },
					new vscode.CancellationTokenSource().token,
					context.secrets,
				);
				vscode.window.showInformationMessage(
					l10n("OpenCode Go model list updated successfully."),
				);
			} catch (error) {
				logger.error("models.update.failed", { error: String(error) });
				vscode.window.showErrorMessage(
					l10n(
						"Failed to update OpenCode Go model list. See output for details.",
					),
				);
			}
		}),
		...
		vscode.commands.registerCommand("opencodego.checkUsage", async () => {
			...
			if (!apiKey) {
				vscode.window.showWarningMessage(
					l10n(
						"No API key configured. Please run the 'OpenCode Go: Set API Key' command first.",
					),
				);
				return;
			}
			...
			if (!usage) {
				if (getUsageFetchStatus() === "unauthorized") {
					vscode.window.showErrorMessage(
						l10n("OpenCode Go usage is unavailable (no active Go plan)."),
					);
				} else {
					vscode.window.showErrorMessage(
						l10n("Failed to fetch OpenCode Go usage. See output for details."),
					);
				}
				return;
			}
			vscode.window.showInformationMessage(
				`OpenCode Go: ${formatUsageSummary(usage)}`,
			);
		}),
	);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("opencodego.openSettings", () => {
			vscode.commands.executeCommand(
				...
				"@ext:OnesoftQwQ.opencode-go-copilot-provider",
			);
		}),
	);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			"opencodego.generateGitCommitMessage",
			async (scm) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("opencodego.setModelPreset", async () => {
			...
			const currentPresetId = config.get<string>(
				"opencodego.modelPreset",
				"custom",
			);
			const currentTemp = config.get<number | null>(
			...
			if (presetId) {
				...
				if (matchedPreset) {
					...
					vscode.window.showInformationMessage(
						l10nFormat(
							"Set to temperature: {0} ({1})",
							String(matchedPreset.temperature),
